    ...
    <main className="flex-1 p-8">
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 mt-1">Tambah akun tim bendahara (tanpa hak Admin/Super Admin)</p>
        </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border overflow-hidden">
        <table className="w-full text-sm">
          <thead className="bg-gray-50 border-b"><tr>{['Email', 'Nama', 'Role', 'Login terakhir', 'Aksi'].map(h => <th key={h} className="text-left px-5 py-3 font-semibold text-gray-600">{h}</th>)}</tr></thead>
          <tbody className="divide-y divide-gray-100">
            {users.length === 0 ? <tr><td colSpan={5} className="text-center py-8 text-gray-400">Memuat / belum ada pengguna</td></tr> :
              users.map(u => {
