...
import { Wallet, TrendingUp, TrendingDown, CalendarDays, CreditCard, Clock, ArrowRight } from 'lucide-react'
      ...
      {loading ? (
        <>
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4 mb-8">
            {[...Array(6)].map((_, i) => <div key={i}>{skeletonCard()}</div>)}
          </div>
        ...
        <>
          {/* Stats Grid */}
          <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4 mb-8">
            <StatsCard icon={Wallet}        title="Total Saldo"             value={formatRupiah(summary?.total_saldo ?? 0)}           color="indigo"  />
            <StatsCard icon={TrendingUp}    title="Pemasukan Bulan Ini"     value={formatRupiah(summary?.pemasukan_bulan_ini ?? 0)}    color="emerald" trend="up" />
            <StatsCard icon={TrendingDown}  title="Pengeluaran Bulan Ini"   value={formatRupiah(summary?.pengeluaran_bulan_ini ?? 0)}  color="rose"    />
            <StatsCard icon={CalendarDays}  title="Pemasukan Tahun Ini"     value={formatRupiah(summary?.pemasukan_tahun_ini ?? 0)}    color="violet"  />
            <StatsCard icon={CreditCard}    title="Pengeluaran Tahun Ini"   value={formatRupiah(summary?.pengeluaran_tahun_ini ?? 0)}  color="amber"   />
            <StatsCard icon={Clock}         title="Menunggu Persetujuan"    value={`${summary?.pengeluaran_pending ?? 0} item`}        color="amber"   />
          </div>
